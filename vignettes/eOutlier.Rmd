...
cleannames_df <- check_names(data = mergealldfs, colsp = 'species', pct = 70, merge = TRUE, verbose = TRUE)
...
multioutliers <- multidetect(data = multipreclened,
                      ...
                      output = 'outlier',
                     exclude = c('x','y'),
                      methods = c('zscore', 'adjbox','iqr', 'semiqr',
                                  'hampel','kmeans', 'mahal',
...
ggoutliers(multioutliers, y=1, raw=TRUE, color = 'purple')
...
labeldata <- extract_cleandata_label(refdata = multipreclened, outliers = multioutliers)
...
* Note: When using **`modelcomparison()`**, the function of **`sdmfit()`**, and **`extract_clean_data()`** are incorporated and therefore the user do not need to do these steps separately.
...
**Visualizing the performance of models before and after removal of outliers.**
