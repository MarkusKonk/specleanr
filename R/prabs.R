...
gen_background <- function(occurences, raster, coords=c('geometry'),
                           class =list(present='P', absent='A'),
                           prop, set.seed=1124){

  if(missing(occurences)){
    stop('Provide species presence data to generate the background records')
  }

  if(missing(raster)){stop('Provide environmental raster layer for point extraction')}

  if(prop<0||prop>1) {
    stop('Prop should be between 0 (not generated) and 1 (equal to length of presence)')
  }
